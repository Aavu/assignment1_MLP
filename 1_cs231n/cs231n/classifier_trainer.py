...
class ClassifierTrainer(object):
  ...
  def train(self, X, y, X_val, y_val, 
            ...
            decay_rate=0.99, update='momentum', sample_batches=True,
      ...
      for p in model:
        ...
        if update == 'sgd':
          ...
          if not p in self.step_cache: 
            self.step_cache[p] = np.zeros_like(grads[p])
          #####################################################################
          ...
          self.step_cache[p] = beta2 * self.step_cache[p] + (1-beta2)*(grads[p]**2)
          dx = - learning_rate * m[p] / (np.sqrt(self.step_cache[p]) + eps)
        ...
        # print(model[p].shape, dx.shape, p)
        model[p] += dx
